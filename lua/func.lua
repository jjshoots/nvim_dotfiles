...
local function on_enter()
  if vim.bo.buftype ~= 'nowrite'
      and vim.bo.buftype ~= 'nofile'
      and vim.bo.buftype ~= 'terminal'
      and vim.bo.buftype ~= 'prompt'
      and vim.bo.buftype ~= 'quickfix' then
...
vim.api.nvim_create_autocmd('FocusGained', {command = 'if exists("g:oldmouse") | let &mouse=g:oldmouse | unlet g:oldmouse | endif', group = group})
...
vim.api.nvim_create_autocmd({'BufNewFile', 'BufRead'}, {pattern = '*.world', command = 'set syntax=xml', group = group})
